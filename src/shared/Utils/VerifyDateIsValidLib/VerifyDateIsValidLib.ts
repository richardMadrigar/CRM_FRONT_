...
export const handleVerifyDateIsValidLib = (date: string) => {
	const isValid = Moment(date, "YYYY-MM-DD", true).isValid();
	return isValid;
};
export const handleVerifyDateAndTimeIsValidLib = (hours: string) => {
	const isValid = Moment(hours, "YYYY-MM-DD HH:mm:ss", true).isValid();
	return isValid;
};
